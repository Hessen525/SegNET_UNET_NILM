...
store = pd.HDFStore('/public/home/jd_ylf/anaconda3_new/envs/nilmtk-env/lib/python3.6/site-packages/nilmtk/disaggregate/classification/ukdale5.h5')
...
def get_series(datastore, house, label, cutoff):
    filename = r'D:\uk2015\house_%1d\labels.dat' %house
    print(filename)
...
for i in range(20):
    ...
    fn = 'SegNet_model_UKDALE_seen_diff_%d.pth' %i
...
for i in range(20):
    ...
    filename = 'SegNet_model_UKDALE_seen_diff_%d.pth' %i
    print(filename)
    model.load_state_dict(torch.load(filename))
    for a in range(3):
