...
for i in range(5):
    ...
    fn = './UNET_model/UKDALE_unseen_%d_linear_d2.pth' %i
...
plt.yscale('log')
plt.grid(True)
plt.savefig('./UNET_model/unseen_loss_linear_d2.png')
plt.show()
...
for i in range(5):
    ...
    filename = './UNET_model/UKDALE_unseen_%d_linear_d2.pth' %i
    print(filename)
    model.load_state_dict(torch.load(filename))
    for a in range(3):
