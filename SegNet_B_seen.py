...
store = pd.HDFStore('/public/home/jd_ylf/ukdale1.h5')
...
for i in range(20):
    ...
    optimizer = optim.Adam(model.parameters(), lr=1.E-4)
    criterion = nn.BCEWithLogitsLoss()
    fn = './SegNet_model_B/SegNet_UKDALE_seen_%d.pth' %i
    model, train_loss, valid_loss, test_loss = train_model(model, batch_size, n_epochs, fn)
...
plt.yscale('log')
plt.grid(True)
#plt.savefig('./SegNet_model_B/seen_loss.png')
...
for i in range(20):
    ...
    filename = './SegNet_model_B/SegNet_UKDALE_seen_%d.pth' %i
    print(filename)
    model.load_state_dict(torch.load(filename))
    for a in range(3):
