...
store = pd.HDFStore('/public/home/jd_ylf/ukdale2.h5')
...
for i in range(20):
    ...
    optimizer = optim.Adam(model.parameters(), lr=1.E-4)
    criterion = nn.BCEWithLogitsLoss()
    fn = './SegNet_model/SegNet_UKDALE_seen_%d.pth' %i
    model, train_loss, valid_loss, test_loss = train_model(model, batch_size, n_epochs, fn)
...
plt.yscale('log')
plt.grid(True)
plt.savefig('./SegNet_model/seen_loss.png')
#
#
#model = PTPNet(1,3,32).cuda()
#print(model.eval())
...
for i in range(20):
    ...
    filename = './SegNet_model/SegNet_UKDALE_seen_%d.pth' %i
    print(filename)
    model.load_state_dict(torch.load(filename))
    for a in range(3):
        #x_true, p_true, s_true, s_hat = evaluate_activation(model, dl_house_total[0], a)
